...
$stmt = $link->prepare("SELECT id, name, hours, minutes, priority, status, categories, description, date FROM activity WHERE MONTH(date) = 7 AND YEAR(date) = ? AND user_id = ?");
...
<script>
    function confirmDelete(activityId) {
        ...
        <script>
                function changeYear() {
                    ...
                    window.location.href = `july.php?year=${selectedYear}`;
            }
        </script>
    ...
    <form method="POST" action="july.php">
