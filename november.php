...
$stmt = $link->prepare("SELECT id, name, hours, minutes, priority, status, categories, description, date FROM activity WHERE MONTH(date) = 11 AND YEAR(date) = ? AND user_id = ?");
    ...
    function confirmDelete(activityId) {
        ...
        <script>
                function changeYear() {
                    ...
                    window.location.href = `november.php?year=${selectedYear}`;
            }
        </script>

        <!-- Left Arrow Button -->
        <a href="october.php">
            <div class="absolute left-[250px] top-[35px] w-[40px] h-[40px] flex items-center justify-center bg-[#E5E9F0] border border-[#1E1E30] rounded-[5px] cursor-pointer">
        ...
        <!-- Right Arrow Button -->
        <a href="december.php">
            <div class="absolute left-[300px] top-[35px] w-[40px] h-[40px] flex items-center justify-center bg-[#E5E9F0] border border-[#1E1E30] rounded-[5px] cursor-pointer">
                <div class="w-0 h-0 border-t-[6px] border-t-transparent border-b-[6px] border-b-transparent border-l-[10px] border-l-[#1E1E30]"></div>
            </div>
        </a>

    </div>
    <form method="POST" action="november.php">
